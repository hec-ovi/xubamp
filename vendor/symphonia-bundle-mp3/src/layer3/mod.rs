...
impl Layer3 {
    ...
    fn read_main_data(
        ...
        for gr in 0..header.n_granules() {
            ...
            for ch in 0..header.n_channels() {
                // xubamp patch: an empty granule channel (part2_3_length == 0) carries neither
                // scale factors nor Huffman-coded samples and consumes no main data. Encoders emit
                // it for a digitally silent channel, which is common at track intros, fades, and
                // gaps. Reading scale factors from it yields part2_len > part2_3_length ("mpa:
                // part2_3_length is not valid"), and when the preceding granule ended exactly on
                // the last byte of main data the reader position is legitimately one past the end
                // ("mpa: invalid main_data offset"). Either error clears the bit reservoir, so
                // every later frame that reuses it fails too and the rest of the track is dropped.
                // Zero the spectrum and move on instead.
                if frame_data.granules[gr].channels[ch].part2_3_length == 0 {
                    requantize::zero(&mut self.samples[gr][ch]);
